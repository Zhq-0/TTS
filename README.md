...
这个仓库整理了三个本地语音合成项目的可公开材料，覆盖 zero-shot 语音克隆、小数据多说话人微调、自动评测和模型对比。仓库包含自写脚本、配置样例、实验报告、指标汇总、局部源码改动 patch，以及 25 条生成样例和 12 条对应参考音频；不包含模型权重、训练数据、checkpoint 或第三方大仓库源码。
...
> 说明：dots.tts 的指标来自同一实验设置下 180 条短文本汇总及中文 60 条子集，不代表单条示例音频；CosyVoice3 指标来自四角色 common-line 汇总；OmniVoice 指标来自 200 条扩展文本评测汇总。
...
除下方列出的试听样例外，本仓库不公开训练数据、其余原始数据或批量生成结果。`assets/audio/` 仅包含表格中列出的 25 条合成展示音频及 12 条对应参考音频。
